...
###CONTAR JUGADORES EN CADA CLUSTER
# Contar jugadores únicos en clusters_2
resumen_clusters_2_UT <- BD.kml_long_utility_2 %>%
  group_by(clusters_2) %>%
  summarize(jugadores_unicos = n_distinct(summonerName))

# Contar jugadores únicos en clusters_5
resumen_clusters_5_UT <- BD.kml_long_utility_2 %>%
  group_by(clusters_5) %>%
  summarize(jugadores_unicos = n_distinct(summonerName))

# Ver resultados
resumen_clusters_2_UT
resumen_clusters_5_UT
